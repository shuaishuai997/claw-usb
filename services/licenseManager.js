...
class LicenseManager {
  ...
  getMachineId() {
    try {
      ...
      if (platform === 'win32') {
        ...
        const drive = appPath.substring(0, 2);
        
        const result = spawnSync('wmic', ['logicaldisk', 'where', `DeviceID="${drive}"`, 'get', 'VolumeSerialNumber'], { encoding: 'utf-8' });
        const serial = result.stdout?.split('\n')[1]?.trim() || '';
  ...
  async getLicenseInfo() {
    console.log('[License] getLicenseInfo called, NODE_ENV:', process.env.NODE_ENV);
    if (process.env.NODE_ENV === 'development') {
      return {
        activated: true,
        licenseKey: 'DEV-MODE-KEY',
